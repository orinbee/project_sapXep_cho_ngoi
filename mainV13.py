...
class SeatArrangementApp:
    ...
    def open_roster_manager(self):
        # ... (open_roster_manager giữ nguyên)
        ...
        def on_double_click(event):
            ...
            def on_save_edit(e): new_value = entry.get(); tree.set(item_id, column_id, new_value); entry.destroy()
            entry.bind("<Return>", on_save_edit); entry.bind("<FocusOut>", on_save_edit)
        tree.bind("<Double-1>", on_double_click)

    def _add_student_to_roster(self, tree):
        # ... (_add_student_to_roster giữ nguyên)
    ...
    def _delete_student_from_roster(self, tree):
        # ... (_delete_student_from_roster giữ nguyên)
    ...
    def _save_roster_changes(self, tree, window):
        # ... (_save_roster_changes giữ nguyên)
        ...
        try:
            for item_id in tree.get_children():
                ...
                new_students_data.append({'Học sinh': name, 'Giới tính': gender, 'Chiều cao': height, 'DiemTB': score, 'GhiChu': notes, 'sit_next_to': old_student_info.get('sit_next_to', []), 'dont_sit_next_to': old_student_info.get('dont_sit_next_to', [])})
            self.students_data = new_students_data; self.students = [s['Học sinh'] for s in self.students_data]
            self._set_dirty(); self._sync_data_and_ui(); self.update_status(f"Đã cập nhật sĩ số lớp. Hiện có {len(self.students)} học sinh."); window.destroy()
    ...
    def load_ui(self):
        # ... (load_ui gần như giữ nguyên, chỉ thêm 1 nút mới)
        main_pane = ttk.PanedWindow(self.root, orient=HORIZONTAL); main_pane.pack(fill=BOTH, expand=True)
        control_panel = ttk.Frame(main_pane, padding=10, width=320); control_panel.pack_propagate(False); main_pane.add(control_panel, weight=1)
        canvas_container = ttk.Frame(main_pane, padding=(0, 10, 10, 0)); main_pane.add(canvas_container, weight=4)

        class_lf = ttk.LabelFrame(control_panel, text=" Quản Lý Lớp Học ", padding=10, bootstyle=PRIMARY); class_lf.pack(fill=X, pady=(0, 15))
        self.class_selector = ttk.Combobox(class_lf, state="readonly", values=[]); self.class_selector.pack(fill=X, pady=(0, 5)); self.class_selector.bind("<<ComboboxSelected>>", self._on_class_selected)
        class_btn_frame = ttk.Frame(class_lf); class_btn_frame.pack(fill=X)
        ...
        self.theme_var = tk.BooleanVar(value=False); theme_switch = ttk.Checkbutton(theme_frame, bootstyle="switch", variable=self.theme_var, command=self.toggle_theme); theme_switch.pack(side=LEFT, padx=10)

        lf1 = ttk.LabelFrame(control_panel, text=" 1. Tải Dữ Liệu Học Sinh ", padding=10, bootstyle=DEFAULT); lf1.pack(fill=X, pady=(0, 10))
        ttk.Button(lf1, text="📂 Tải File Excel cho Lớp Này", command=self.load_students, bootstyle=DEFAULT).pack(fill=X)
        ...
        ttk.Label(lf2, text="Số tổ:").grid(row=0, column=0, padx=5, pady=5, sticky=W); self.team_spinbox = ttk.Spinbox(lf2, from_=1, to=20, width=5); self.team_spinbox.grid(row=0, column=1, padx=5, pady=5, sticky=W); self.team_spinbox.set(self.num_teams)
        ttk.Label(lf2, text="Số bàn mỗi tổ:").grid(row=1, column=0, padx=5, pady=5, sticky=W); self.table_spinbox = ttk.Spinbox(lf2, from_=1, to=30, width=5); self.table_spinbox.grid(row=1, column=1, padx=5, pady=5, sticky=W); self.table_spinbox.set(self.num_tables)
        ttk.Button(lf2, text="Áp dụng cấu hình", command=self.apply_team_table_config, bootstyle="secondary-outline").grid(row=2, column=0, columnspan=2, pady=10, sticky=EW)

        lf3 = ttk.LabelFrame(control_panel, text=" 3. Sắp Xếp & Tùy Chỉnh ", padding=10, bootstyle=DEFAULT); lf3.pack(fill=X, pady=10)
        ttk.Button(lf3, text="🎲 Tùy chọn Sắp xếp", command=self.show_sort_options).pack(fill=X, pady=(0,5))
        ...
        ttk.Button(lf4, text="Xuất Sơ Đồ ra Excel", command=self.save_results, bootstyle="info-outline").pack(fill=X, pady=(0, 5))
        ttk.Button(lf4, text="🖨️ Xuất Sơ Đồ ra PDF (Để in)", command=self.export_to_pdf, bootstyle="info").pack(fill=X, pady=(5, 5))
        ttk.Button(lf4, text="Chụp Ảnh Sơ Đồ", command=self.save_as_image, bootstyle="info-outline").pack(fill=X)
        
        lf5 = ttk.LabelFrame(control_panel, text=" Công Cụ Khác ", padding=10, bootstyle=DEFAULT); lf5.pack(fill=X, pady=10)
        ttk.Button(lf5, text="🎨 Đổi Màu Tổ", command=self.change_team_colors, bootstyle="secondary-outline").pack(fill=X, pady=(0, 5))
        ttk.Button(lf5, text="📜 Xem Lịch Sử Sắp Xếp của Lớp", command=self.view_history, bootstyle="secondary-outline").pack(fill=X, pady=(5,0))
        ttk.Button(lf5, text="✨ Gọi Tên Ngẫu Nhiên", command=self.pick_random_student, bootstyle="success").pack(fill=X, pady=(5, 0))
        
        # <<--- MỚI: THÊM NÚT KÍCH HOẠT TÍNH NĂNG AI --- >>
        ttk.Button(lf5, text="📊 Phân tích Tương tác Lớp học", command=self.analyze_social_network, bootstyle="info").pack(fill=X, pady=(5, 0))
        ttk.Button(lf5, text="🤖 AI Gợi Ý Chia Nhóm Học Tập", command=self.open_group_creation_tool, bootstyle="primary").pack(fill=X, pady=(5, 0))
    ...
    def open_ai_report_window(self):
        ...
        report_widget = scrolledtext.ScrolledText(container, wrap=tk.WORD, font=("Arial", 11), state=tk.DISABLED)
        report_widget.pack(fill=BOTH, expand=True, pady=5)
    ...
    def _generate_ai_report(self):
        ...
        if len(history) < 2:
            return "Chưa có đủ dữ liệu lịch sử để phân tích. \n\nVui lòng sử dụng tính năng 'Tích điểm thi đua' và lưu sơ đồ nhiều lần để AI có dữ liệu học tập."
